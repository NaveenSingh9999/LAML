...
create_release_directory() {
    ...
    rm -rf release
    mkdir -p "release/laml-v3.0.0"/{windows,linux,termux,vscode-extension}
...
build_vscode_extension() {
    ...
    if command -v npm >/dev/null 2>&1; then
        ...
        # Package extension
        vsce package --no-yarn --out "../$release_dir/vscode-extension/laml-3.0.0.vsix"
...
main() {
    ...
    create_release_directory
    local release_dir="release/laml-v3.0.0"
    ...
    cd release
    tar -czf "laml-v3.0.0.tar.gz" "laml-v3.0.0/"
    zip -r "laml-v3.0.0.zip" "laml-v3.0.0/"
    cd ..
    
    echo ""
    print_colored $GREEN "🎉 LAML v3.0.0 build completed successfully!"
    echo ""
    print_colored $CYAN "📦 Release packages:"
    print_colored $NC "• $release_dir/ - Complete installer package"
    print_colored $NC "• release/laml-v3.0.0.tar.gz - Linux/macOS archive"
    print_colored $NC "• release/laml-v3.0.0.zip - Windows archive"
    echo ""
